...
- `systeminformer-update.json`：内置更新器读取的签名元数据，以及上一 Release 到本次 Release 之间的全部提交记录
...
更新日志直接来自发布提交的本地 Git 历史，并随更新元数据发布。发布流程读取上一条已发布
Release 元数据中的提交作为边界，记录其后到本次发布提交之间的全部提交，不设置条数上限。
客户端查看日志时复用检查版本所下载的同一份元数据，不再二次请求 `latest`，也不再调用有匿名
限额的 GitHub commits API，因此既不会因共享出口触发 API 限流，也不会在检查后显示另一版本的日志。
...
确认改动已提交并推送到 `zh-CN`，且分支上的轻量检查成功后，手动发布自己的汉化改动：
...
官方上游出现新的正式版本 Tag 时，`zh-CN upstream check and release` 会自动执行同步候选、汉化检查、
合并和发布。只有检查通过的上游 Tag 才会进入 `zh-CN`；检查失败时不会发布，下一次定时任务会重试。
...
上一 Release 提交边界、完整提交范围、下载地址、长度、哈希和更新器签名长度，全部通过后才把草稿公开。任何一步失败都不会
把未验证的 Release 暴露给更新器；本次运行创建的未公开草稿会被清理，修复问题后重新运行即可。

因此汉化文本、规则或测试有改动时不必等待上游发布新版本：提交并推送改动，等轻量检查通过，
再手动运行一次 `zh-CN build` 即可向已安装版本发布更新。上游新正式 Tag 则由定时工作流自动发布。
